...
export default function LegacyPage() {
  ...
  function buildQuery(kind: Strategy) {
    ...
    // broad / chips
    return normalizeSpaces(
      [iconicLabel, brand, category, color, material, ...on].filter(Boolean).join(" ")
    );
  }

  const activeQuery = useMemo(() => {
    return buildQuery(strategy);
    // buildQuery is defined in this component; we intentionally don't include it in deps.
  ...
  async function downscaleImage(file: File): Promise<File> {
    // Same behavior as the original demo, with a Safari-safe fallback.
    const MAX_EDGE = 1600;
    const QUALITY = 0.85;
            ...
            {chips.map((c) => (
              ...
              >
                ...
                <button
                  ...
                  style={{ all: "unset", cursor: "pointer", opacity: 0.8, fontWeight: 900 }}
                  title="delete"
                >
                  ×
                </button>
